...
export type Game={version:2;date:string;archive:boolean;draft:string;attempts:Attempt[];status:Status;hintUsed:boolean;updatedAt:string};
...
export const newGame=(date:string,archive=false):Game=>({version:2,date,archive,draft:'',attempts:[],status:'playing',hintUsed:false,updatedAt:new Date().toISOString()});
